    ...
    for bool_2 = 0:1
        for bool_3 = 0:1
            ...
            for j = 1:N
                ...
                if bool_3 == 0
                    stress(1,j,bool_1+1,bool_2+1,bool_3+1) = abs(dot(grad3_6, vibrate));
                else
                    stress(1,j,bool_1+1,bool_2+1,bool_3+1) = abs(dot(grad3_5, vibrate));
                end
                
                % left_displacement(i) = norm(T(9:10,:) * vibrate);
                % right_displacement(i) = norm(T(11:12,:) * vibrate);
            end
        end
    end
end
...
subplot(4,2,1)
plot(freq, abs(stress(:,:,1,1,1)))
ylim([0,min(20,0.1/c)]);
title('模态000应力')

subplot(4,2,2)
plot(freq, abs(stress(:,:,1,1,2)))
ylim([0,min(20,0.1/c)]);
title('模态001应力')

subplot(4,2,3)
plot(freq, abs(stress(:,:,1,2,1)))
ylim([0,min(20,0.1/c)]);
title('模态010应力')

subplot(4,2,4)
plot(freq, abs(stress(:,:,1,2,2)))
ylim([0,min(20,0.1/c)]);
title('模态011应力')

subplot(4,2,5)
plot(freq, abs(stress(:,:,2,1,1)))
ylim([0,min(20,0.1/c)]);
title('模态100应力')

subplot(4,2,6)
plot(freq, abs(stress(:,:,2,1,2)))
ylim([0,min(20,0.1/c)]);
title('模态101应力')

subplot(4,2,7)
plot(freq, abs(stress(:,:,2,2,1)))
ylim([0,min(20,0.1/c)]);
title('模态110应力')

subplot(4,2,8)
plot(freq, abs(stress(:,:,2,2,2)))
ylim([0,min(20,0.1/c)]);
title('模态111应力')
